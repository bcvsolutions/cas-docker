...
truststorepath="$CAS_CONFIG/truststore.jks";
...
if [ ! -f "truststore.jks" ]; then
  echo "Creating truststore $CAS_CONFIG";
  ...
  openssl req -new -key fakecert.key -out fakecert.csr -subj "/C=CZ/ST=Czech Republic/L=Prague/O=BCV/CN=Cas placeholder cert";
...
echo "Found truststore $CAS_CONFIG";

if [ ! -z "$(ls -A $CAS_START/trustcert)" ]; then
  # trustcert directory is not empty, we import certificates
  storepass=$(cat truststore.pwfile);
  for crtfile in $CAS_START/trustcert/*; do
  ...
  for crtalias in $(keytool -list -keystore truststore.jks -storepass "$storepass" | grep -v "^placeholder-cert" | awk -F, '$0~/trustedCertEntry/ {print $1}'); do
    ...
    if [ ! -f "$CAS_START/trustcert/$crtalias" ]; then
